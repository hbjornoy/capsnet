...
from model import capsules, capsules_color, ConvNet_DR_paper
from loss import SpreadLoss
from datasets import smallNORB, Segmented_seedlings
...
parser.add_argument('--cuda-device', type=int, default=0, metavar='N',
                    help='choose GPU devicenumber to train on')
...
parser.add_argument('--dataset', type=str, default='mnist', metavar='D',
                    help='dataset for training(mnist, smallNORB, segmentedSeedlings)')
parser.add_argument('--model', type=str, default='capsule', metavar='D',
                    help='models for training(capsule, convnet)')
...
def get_setting(args):
    ...
    if torch.cuda.is_available():
        ...
        print("Selected cuda_device: ", args.cuda_device)
        print("current active device: ", torch.cuda.current_device())
    ...
    if args.dataset == 'mnist':
    ...
    elif args.dataset == 'segmentedSeedlings':
        num_class = 12
        train_loader = torch.utils.data.DataLoader(
            Segmented_seedlings(path, train_val_test='train', download=True,
                      transform=transforms.Compose([
                          transforms.Resize(48),
                          transforms.RandomCrop(32),
                          transforms.ToTensor()
        ...
        val_loader = torch.utils.data.DataLoader(
            Segmented_seedlings(path, train_val_test='val',
                      transform=transforms.Compose([
                          transforms.Resize(48),
                          transforms.RandomCrop(32),
                          transforms.ToTensor()
            ...
            batch_size=args.batch_size, shuffle=True, **kwargs)
        test_loader = torch.utils.data.DataLoader(
            Segmented_seedlings(path, train_val_test='test',
                      transform=transforms.Compose([
                          transforms.Resize(48),
                          transforms.RandomCrop(32),
                          transforms.ToTensor()
...
def main():
    ...
    print('Device to run on: ', device)
    
    # datasets
    num_class, train_loader, test_loader = get_setting(args)

    if args.model == 'convnet':
        A, B, C, D, E = 256, 256, 128, 328, 192
        K, padding, channels, nb_classes = 5, 0, 3, 12
        model = ConvNet_DR_paper(A=A, B=B, C=C, D=D, E=E, 
                            K=K, padding=padding, channels=channels, nb_classes=nb_classes).to(device)
    else:

        # CapsuleModel
        A, B, C, D = 128, 32, 32, 64     # HBmodel
        # A, B, C, D = 64, 8, 16, 16   #original
        # A, B, C, D = 32, 32, 32, 32  #old
        model = capsules_color(A=A, B=B, C=C, D=D, E=num_class,
